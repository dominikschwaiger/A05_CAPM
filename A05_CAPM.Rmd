...
subtitle: "Submit until Monday 2020-10-19, 13:00"
...
We get the data and transmute it to returns. 
```{r}
...
stock.prices
```
Next, we create monthly returns. 
```{r}
...
Group B: 20% - 70%
Group C: 0%  - 50%
So the groups we created have to add up to the weight intervals we definded
```{r}
...
Next, we chart the efficient frontiers. 
```{r}
...
1.35, 0.97, 0.92, 0.69, 0.41, 1.13, 1.55, 1.31, 1, 1.03

Now, we add the constraint and combine them in a list. 
```{r}
exp_constr <- factor_exposure_constraint(assets=stockselection,B=c(1.35, 0.97, 0.92, 0.69, 0.41, 1.13, 1.55, 1.31, 1, 1.03),lower=0.6, upper=0.9)
